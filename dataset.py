...
class SiburDataset(Dataset):
    ...
    def train_od(self):
        print('generate outliers data...')
        data = []
        for i in tqdm(range(self.__len__())):
            row = self.__getitem__(i, precompile=True)
            row = row[0].numpy().flatten()
            data.append(row.reshape(1, -1))

        data = np.concatenate(data, axis=0)

        if self.task == 'train':
            copod = COPOD().fit(data)
            self.outliers = copod.decision_scores_
            dump(copod, ABS_PATH.joinpath('copod.joblib'))
        else:
            copod = load(ABS_PATH.joinpath('copod.joblib'))
            self.outliers = copod.decision_function(data)
    ...
    def __getitem__(self, index, precompile=False):
        ...
        if not precompile:
            vector = np.concatenate([vector, [self.outliers[index]]])
